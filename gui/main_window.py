...
class MainWindow(DragDropMixin):
    ...
    def setup_ui(self):
        ...
        # URL input section
        url_frame = ttk.LabelFrame(main_frame, text="Add URLs or Files", padding="5")
        ...
        ttk.Button(url_frame, text="Add URL", command=self.add_url).grid(row=0, column=1)
        ttk.Button(url_frame, text="Add Files", command=self.add_files).grid(row=0, column=2, padx=(5, 0))
        
        # Drag and drop area
        self.drop_frame = tk.Frame(main_frame, bg="lightgray", relief="sunken", bd=2, height=80)
        ...
        # Queue display
        queue_frame = ttk.LabelFrame(main_frame, text="Processing Queue", padding="5")
        ...
        ttk.Button(control_frame, text="Clear Completed", command=self.clear_completed).pack(side=tk.LEFT)
        ttk.Button(control_frame, text="Clear All", command=self.clear_all).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Button(control_frame, text="Settings", command=self.open_settings).pack(side=tk.RIGHT)
